...
def build_story():
    ...
    s.append(li("<b>事前に座席を確認するモード</b>。先に席を決めて調整しておき、教室ではそのとおりに決まるようルーレットを回せる"))
    ...
    s.append(p("7.6 事前に座席を確認する（配慮が必要な生徒が多いとき）", "h2"))
    s.append(p(
        "職員室で先に席を決めて調整しておき、教室ではその座席になるようルーレットを回せるモードです。"
        "配慮が必要な生徒が多く、その場の抽選だけでは不安なときに使います。", "body"
    ))
    s.append(p("設定タブの「🔒 事前に座席を確認する」にチェックを入れると使えます。", "body"))
    s.append(p("＜職員室での準備＞", "h3"))
    s.append(li("上部の「⚡ 瞬時に決定」を押して席を決めます（ルーレットは教室で回すので、ここでは使いません）"))
    s.append(li("「🔄 入替」で、配慮が必要な生徒の席を調整します"))
    s.append(li("上部の「✅ この座席で確定」を押します"))
    s.append(p(
        "確定すると<b>座席表は空席の表示に戻ります</b>。いつもの席替え前とまったく同じ見た目なので、"
        "教室でモニターに映しても、これから決まるように見えます。", "note"
    ))
    s.append(p("＜教室での実施＞", "h3"))
    s.append(li("いつもどおり「🎰 一人ずつルーレット」「🎲 一括ルーレット」を回します"))
    s.append(li("演出はそのままに、結果は<b>事前に確定した座席</b>のとおりに決まります"))
    s.append(li("最後まで終わると確定は自動で解除され、次の席替えはいつもどおりになります"))
    s.append(p("＜確認・やり直し＞", "h3"))
    s.append(li("設定タブの「👁 確定した座席を確認する」で中身を確認できます（教室で映す前に「🙈 隠す」を押してください）"))
    s.append(li("「✏ 確定を解除して調整し直す」を押すと座席表に戻るので、入替でやり直せます"))
    s.append(p(
        "確定した後に名簿や座席レイアウトを変えると、その座席では決められなくなるため警告が出ます。"
        "また、確定を消してしまう操作（「⚡ 瞬時に決定」など）のときは、先に確認が出ます。", "warn"
    ))
